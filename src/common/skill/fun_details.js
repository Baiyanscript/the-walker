...
export const detail_LIB = {
    ...
    // -------- 七咒之戒(混沌预设, 需求.md 2026-08-16) --------
    "effect_sevenCurses": (eff, o, SD) => {
        if (SD) return `七咒之戒(混沌常驻): \n` +
            `1.任意敌人的攻击等效计算power+1 [待实现] \n` +
            `2.任意获得的护盾量减半(向上取整) [待实现] \n` +
            `3.任意造成的攻击等效power-1 \n` +
            `4.过度劳累: 打出牌时额外消耗1点AP(不扣到负数) \n` +
            `5.暗淡的火光: 篝火回血量降至最大血量30%, 满血上限提升削弱到5% \n` +
            `6.前途渺茫: 地图不再出现纯奖励关卡, 替换为纯战斗关卡 \n` +
            `7.生死之渺: 不死图腾(超越生死)的效果失效 \n` +
            `祝福① 微弱的火焰: 篝火随机强化你的一项属性 \n` +
            `祝福② 更高概率得到更高级别的卡牌`
        return `七咒之戒`
    }
}
