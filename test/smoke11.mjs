...
import { addEffect, fireEffect } from "./.cache/esm/core/effect.mjs"
...
// 模拟 fighting.ux 玩家出牌流程: 构建ctx -> when_act全量扫描([玩家,...怪物]) -> 重算targetIndex -> 执行技能
...
fireEffect({ trigger: "when_act", targets: [p3, ...mobs3], exDate: { ctx: ctx3, buildSkillCtx }, mobList: mobs3, playerInfo: p3 })
...
fireEffect({ trigger: "when_act", targets: [p4, ...mobs4], exDate: { ctx: ctx4, buildSkillCtx }, mobList: mobs4, playerInfo: p4 })
...
fireEffect({ trigger: "when_act", targets: [p5, ...mobs5], exDate: { ctx: ctx5, buildSkillCtx }, mobList: mobs5, playerInfo: p5 })
