...
// 模拟 fighting.ux 玩家出牌流程: 构建skillCtx -> when_act(扫玩家) + when_player_act(扫怪物) -> 重算targetIndex -> 执行技能
function simulatePlayerAct(player, mobs, card, targetIdx) {
  ...
  skillCtx.targetIndex = mobs.indexOf(skillCtx.target)
  for (const s of (skillCtx.source.doSkill || [])) runSkill(s, skillCtx)
  return skillCtx
}
...
const skillCtx3 = buildSkillCtx({ source: createCard("斩击", { level: 1 }), actor: p3, target: p3, targetIndex: -1, playerInfo: p3, mobList: mobs3, handPool: [], drawPool: [] })
fireEffect({ trigger: "when_player_act", targets: mobs3, exDate: { skillCtx: skillCtx3, buildSkillCtx }, mobList: mobs3, playerInfo: p3 })
check("目标(玩家)不在怪物组, 不重定向", () => assert.equal(skillCtx3.target, p3))
...
const skillCtx4 = buildSkillCtx({ source: createCard("斩击", { level: 1 }), actor: p4, target: mobs4[0], targetIndex: 0, playerInfo: p4, mobList: mobs4, handPool: [], drawPool: [] })
fireEffect({ trigger: "when_player_act", targets: mobs4, exDate: { skillCtx: skillCtx4, buildSkillCtx }, mobList: mobs4, playerInfo: p4 })
check("目标已是自己, 保持指向", () => assert.equal(skillCtx4.target, mobs4[0]))
...
const skillCtx5 = buildSkillCtx({ source: createCard("斩击", { level: 1 }), actor: p5, target: mobs5[0], targetIndex: 0, playerInfo: p5, mobList: mobs5, handPool: [], drawPool: [] })
fireEffect({ trigger: "when_act", targets: p5, exDate: { skillCtx: skillCtx5, buildSkillCtx }, mobList: mobs5, playerInfo: p5 })
fireEffect({ trigger: "when_player_act", targets: mobs5, exDate: { skillCtx: skillCtx5, buildSkillCtx }, mobList: mobs5, playerInfo: p5 })
Math.random = origRandom
check("狂乱把目标改史莱姆后, 替罪羊将其拉回哥布林", () => assert.equal(skillCtx5.target, mobs5[1]))
