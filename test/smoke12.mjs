// smoke12: 固定层数脚本(getLevelScript: 全局/角色合并, 校验失败回退null, rewardLevel hard展开)
...
check("49层入口类型齐备", () => {
  const keys = s49.nodes.map(n => n.rewardType).sort()
  assert.deepEqual(keys, ["商店", "获得卡牌", "强化卡牌", "融合卡牌", "篝火", "遗物"].sort())
})
check("49层 rewardLevel 均为 hard", () => s49.nodes.every(n => n.rewardLevel === "hard"))
...
check("富二代少爷第1层命中: 商店节点", () => {
  assert.ok(s1Gambler)
  assert.deepEqual(s1Gambler.nodes.map(n => n.rewardType), ["商店"])
})
...
preset_LIB["战士"].levelScript = { 49: { nodes: [{ rewardType: "非法事件" }] } }
check("非法 rewardType -> null", () => assert.equal(getLevelScript(49, "战士"), null))
...
delete preset_LIB["战士"].levelScript
preset_LIB["战士"].levelScript = { 50: { nodes: [{ rewardType: "商店", mobSet: [{ addMob: [{ key: "不存在的怪" }] }] }] } }
